...
- Settings management (`src/settings.js`, `src/settings-ui.js`, `src/confirm.js`) — `createSettings(db)` engine reads/writes `sync_anchor_date` in Dexie `settings` store, counts/prunes `daily_records` before a given date, counts all records, and wipes the full database; `createSettingsUI(doc, settings, reporter, confirmFn)` renders `#settings-modal` with 📅 SYNC BOUNDARY / 🗑️ DATA PURGE OPTIONS sections (anchor-date picker, impact preview, prune/wipe actions, auto-save anchor on date change); `createConfirmAdapter(windowRef)` is an injectable seam replacing `window.confirm`; settings modal is opened via `#settings-btn`; successful mutations dispatch `data:records:mutated`; (ST-009) also reads/writes the user's Home Base City (`home_base_city` key, validated against `src/odyssey.js`'s `HOME_BASE_CITIES`) for the Odyssey Lab section
- Analytics / Gamification / Odyssey Lab (`src/analytics.js`, `src/analytics-ui.js`, `src/gamification.js`, `src/gamification-ui.js`, `src/odyssey.js`, `src/odyssey-ui.js`) — a new 🧪 Lab tab (`data-tab="lab"` → `#tab-lab`) presenting three read-only sections computed purely from `daily_records`: an Analytics dashboard (Hall of Fame lifetime metrics, Top 5 days, day-of-week and 24-hour distributions, year-selectable monthly breakdown); a Gamification RPG level (XP from lifetime steps, 50-level rank ladder, four achievement trophies — Centurion/Marathoner/Unstoppable/Night Owl); and an Odyssey virtual-expedition progression bar (6 milestone legs from a user-selectable Home Base City); ST-009; see `.context/flows/analytics-lab-dashboard.md`, `.context/flows/gamification-levels-trophies.md`, `.context/flows/odyssey-virtual-expedition.md`
