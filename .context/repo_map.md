...
- UI event handlers (bound in `src/main.js`):
  ...
  - `data:records:mutated` custom event → `progressUI.render()`, `streakUI.render()`, `calendarUI.render()`, `searchLabUI.render()` (fail-open, registered in `src/main.js`)
  - `ui:open-day-drawer` custom event → `calendarUI.openDrawerForDate(date)` (registered in `src/main.js`)
- `DOMContentLoaded` → `bootstrap()` also calls `progressUI.render()`, `streakUI.render()`, `calendarUI.render()`, and `searchLabUI.render()` on load
- Goal changes invoke progress rendering and the streak render callback; sync completion renders progress, streak, calendar, and search-lab cards
...
- Calendar renderer: `src/calendar-ui.js` (`createCalendarUI(doc, db, calendarEngine, reporter, records, processImage)` factory; `render()` builds `#calendar-nav`, `#calendar-summary`, `#calendar-grid`, and `#day-drawer` into `#tab-calendar`; override form + revert button injected into drawer when `records` is provided; `openDrawerForDate(date)` public method opens the drawer for a given date programmatically)
...
- Search Lab analytics engine: `src/search-lab.js` (`createSearchLab(db, goal)` DI factory; `findNearMisses()` — returns days where `effectiveDistanceKm ∈ [target×0.90, target)`; `computeDayOfWeekSlump()` — 7-bucket Mon–Sun hit-rate/step/distance aggregates with `primarySlump` flag; `comparePeriods(rangeA, rangeB)` — aggregates two date-range windows and returns `{ periodA, periodB, deltas }`; pure helpers exported: `isNearMiss`, `dayOfWeekIndex`, `dateBounds`, `computeComparisonDelta`)
- Search Lab UI renderer: `src/search-lab-ui.js` (`createSearchLabUI(doc, engine, reporter)` DI factory; `render()` builds near-miss card, day-of-week slump card, and period-comparison card into `#tab-search`; dispatches `ui:open-day-drawer` custom event on near-miss row clicks)
...
## Testing Surfaces
- Unit tests: `src/*.test.js` (Vitest 4, jsdom) — auth, config, db, storage, tabs, ui-status, main, steps, styles, docs, goal, progress, progress-ui, streak, streak-ui, goal-history, calendar, calendar-ui, records, image-processor, search-lab, search-lab-ui
