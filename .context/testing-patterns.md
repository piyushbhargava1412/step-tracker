...
| ✅ DONE  | `createCalendarUI` (`src/calendar-ui.js`)    | Grid/summary/nav/drawer render, fail-open, idempotent re-render, drawer dismissal (close btn / overlay / Escape), focus restoration, mounts shared override form (`src/override-form.js`), revert button, `data:records:mutated` dispatch |
| ✅ DONE  | `createOverrideForm` / `createProofLightbox` (`src/override-form.js`) | Shared override form (`mount(container, { date, record }, { signal })` — steps input, mandatory proof, Save Override) + proof lightbox (`open(src, panel)`/`close()`), validation guards, signal-bound listener cleanup, `data:records:mutated` dispatch, `[override-form]` logging (`src/override-form.test.js`); reused by calendar drawer and Search Lab Edit Day |
...
| ✅ DONE  | `createSearchUI` (`src/search-ui.js`)              | Render skeleton (7 data-field controls, action buttons, results grid, summary card, export controls), idempotent re-render, AbortController cleanup, delegated execute/reset/export-csv/export-json/edit-day actions, retained-query replay on re-render, mounts shared override form on missed rows when records injected, error path → reporter.db, no-innerHTML contract (`src/search-ui.test.js`) |
