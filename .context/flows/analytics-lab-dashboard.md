...
2. `computeLifetimeMetrics(records, activeStepGoal)` sums `effective_steps`/`effective_distance_km` and computes `dailyAverage`; `longestStreak` **delegates to `computeToleranceStreaks`** (from `src/streak.js`) rather than re-implementing streak logic, using today (UTC) as the reference date.
3. `computeTopRecords(records, n=5)` sorts a shallow copy descending by `effective_steps` and slices the top N.
4. `computeDayOfWeekDistribution(records)` buckets by ISO weekday (0=Monday…6=Sunday, converted from `Date.getDay()`), averages per slot, and picks `powerDay`/`lazyDay` with lowest-index-wins tie-breaking.
...
8. All bar charts share one generic `_buildBarChart(values, labels, unit)` builder — proportional `<div>` widths against `Math.max(...values, 1)` (never `/0`), each bar carrying an `aria-label`.
...
## Data Touchpoints
- **Entities**: `daily_records` (`date`, `effective_steps`, `effective_distance_km`, `hourly_steps`, `screenshot_proof`); `settings` row `key = 'active_step_goal'` (read-only, for the streak-goal input to `computeToleranceStreaks`)
...
- `styles.css` — `.hof-grid`, `.bar-chart`/`.bar-chart__bar` (shared with Gamification's RPG progress bar styling area), Lab panel section styles
